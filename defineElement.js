/* document.defineElement build 0.1.10 (https://github.com/jochemstoel/document.defineElement) */
(function(window, polyfill) {
    ...
    if (typeof polyfill !== 'object') polyfill = {
        type: polyfill || 'auto'
    };
    var
        REGISTER_ELEMENT = 'defineElement',
        ...
        setPrototype = sPO || (hasProto ? function(o, p) {
            ...
            function setProperties(o, p) {
                for (var
                        key, names = gOPN(p), i = 0, length = names.length; i < length; i++) {
                    key = names[i];
                    if (!hOP.call(o, key)) {
        ...
        onSubtreeModified, callDOMAttrModified, getAttributesMirror, observer, observe, patchIfNotAlready, patch;
    if (!V0) {
        if (sPO || hasProto) {
        ...
        if (IE8) {
            ...
            if (doesNotSupportDOMAttrModified) {
                onSubtreeModified = function(e) {
                    var
                        node = this,
                        oldAttributes, newAttributes, key;
                    if (node === e.target) {
                ...
                getAttributesMirror = function(node) {
                    for (var
                            attr, name, result = {}, attributes = node.attributes, i = 0, length = attributes.length; i < length; i++) {
                        attr = attributes[i];
                        name = attr.name;
        ...
        document[REGISTER_ELEMENT] = function defineElement(type, options) {
                ...
                if (MutationObserver) {
                    observer = (function(attached, detached) {
                        ...
                        return new MutationObserver(function(records) {
                            for (var
                                    current, node, newValue, i = 0, length = records.length; i < length; i++) {
                                current = records[i];
            ...
            if (-2 < (indexOf.call(types, PREFIX_IS + upperType) +
        ...
        document.createElement = (patchedCreateElement = function(localName, typeExtension) {
            var
                ...
                name = '' + localName,
                i = indexOf.call(types, (is ? PREFIX_IS : PREFIX_TAG) +
                    (is || name).toUpperCase()),
                setup = -1 < i;
            if (is) {
    ...
    function purge() {
        for (var
                node, i = 0, length = targets.length; i < length; i++) {
            node = targets[i];
